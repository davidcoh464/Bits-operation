class BitsSort {
    void binarySort(int* arr, int start, int end, int bitOn) {
        ...
        binarySort(arr, start, replace, bitOn);
        //Sorts all numbers from 'replace + 1' until the 'end' that 'bitOn' is 1
        binarySort(arr, replace + 1, end, bitOn);
    }
public:
    void binarySort(int* arr, int size) {
        ...
        binarySort(arr, 0, endNeg, 0x40000000);
        //Sorting the non negative numbers that are from place 'endNeg' + 1 until the end
        binarySort(arr, endNeg + 1, size - 1, 0x40000000);
    }
};
